	...
	for (int i = 0; i < width; i++) {
		for (int j = 0; j < height; j++) { //At each pixel...
			/*
			   ...
			   */
			//Vignette Effect
			//(0,0) is the top left corner of the image. Y axis goes down (0,1) is below (0,0) in the image
			int center_x = width / 2;
			int center_y = height / 2;
			//double distance = sqrt(pow(i - center_x, 2) + pow(j - center_y, 2));
			double distance = hypot(i - center_x, j - center_y); //pythagorean theorem
			//double max_distance = sqrt(pow(center_x, 2) + pow(center_y, 2));
			double max_distance = hypot(center_x, center_y); //If you move the center change this
...
void filter2(unsigned char *in, unsigned char *out) {
	//Do nothing filter
	//Copy and paste another filter here to make them both work
	for (int i = 0; i < width; i++) {
		for (int j = 0; j < height; j++) { //At each pixel...
			out[R(i, j)] = in[R(i, j)];
			out[G(i, j)] = in[G(i, j)];
