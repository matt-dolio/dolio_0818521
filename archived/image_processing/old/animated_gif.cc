...
void pixelize(unsigned char in[], unsigned char out[], unsigned char BLOCKSIZE) {
	//Every BLOCKSIZE pixels we copy the top-left pixel over all the other pixels in a BLOCKSIZExBLOCKSIZE block
    for (int i = 0; i < width; i+=BLOCKSIZE) {
        for (int j = 0; j < height; j+BLOCKSIZE) { //At each pixel...
