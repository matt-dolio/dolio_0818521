...
void damage(int &health, int damage, int &lives, int attacking_type, int defending_type) {
	...
	if (attacking_type == FIRE) {
		if (defending_type == FIRE) damage /= 2.0;
		if (defending_type == GRASS) damage *= 2;
		if (defending_type == AGUA) damage /= 2.0;
	}
	else if (attacking_type == GRASS) {
		if (defending_type == FIRE) damage /= 2.0;
		if (defending_type == GRASS) damage /= 2.0;
		if (defending_type == AGUA) damage *= 2;
	}
	else if (attacking_type == AGUA) {
		if (defending_type == FIRE) damage *= 2;
		if (defending_type == GRASS) damage /= 2.0;
		if (defending_type == AGUA) damage /= 2.0;
	}
